...
parser.add_argument('--no_clutter',
                    action='store_true',
                    help='If true, do not count the clutter class')
parser.add_argument(
    '--visu',
    action='store_true',
...
BLOCK_SIZE = float(FLAGS.block_size)
STRIDE = float(FLAGS.stride)
if not os.path.exists(DUMP_DIR): os.mkdir(DUMP_DIR)
LOG_FOUT = open(os.path.join(DUMP_DIR, 'log_evaluate.txt'), 'w')
LOG_FOUT.write(str(FLAGS) + '\n')
FILE_PATH_LIST = [
    os.path.join(ROOT_DIR, line.rstrip())
    for line in open(FLAGS.room_data_filelist)
]
...
def EvalCustom():
    ...
    for filepath in FILE_PATH_LIST:
        ...
        a, b = eval_one_epoch_custom(sess, ops, filepath, out_data_label_filename,
...
def eval_one_epoch_custom(sess, ops, room_path, out_data_label_filename, out_gt_label_filename):
  ...
  # Get room dimension..
  data_label = loaddatatourn.loadDataRaw(room_path)
  data = data_label[:,0:6]
